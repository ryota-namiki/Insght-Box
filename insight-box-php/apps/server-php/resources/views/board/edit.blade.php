...
@section('title', 'ボード編集 - Insight-Box')

@section('content')
<div class="mb-6">
  <h1 class="text-[28px] font-semibold text-text-main leading-tight">ボードを編集</h1>
  <p class="mt-2 text-base text-text-subtle leading-relaxed">ボードの名前や説明を変更します</p>
</div>
...
<div class="bg-surface rounded-card shadow-card p-6 border border-divider">
  <form action="{{ route('board.update', $board['id']) }}" method="POST">
    ...
    <div class="flex items-center justify-between">
      <a href="{{ route('board.show', $board['id']) }}" class="flex items-center gap-2 text-text-subtle hover:text-text-main transition-fast">
