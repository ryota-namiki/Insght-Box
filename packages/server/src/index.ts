...
declare global {
  ...
  var documents: Map<string, Document> | undefined;
  // eslint-disable-next-line no-var
  var jobs: Map<string, Job> | undefined;
  // eslint-disable-next-line no-var
  var hybridResults: Map<string, HybridOCRResult> | undefined;
}
...
function createTempFilePath(prefix: string, extension = '.png'): string {
...
function updateJobProgress(jobId: string | undefined, progress: number): void {
...
async function createRotatedImageCopy(filePath: string, angle: number): Promise<string> {
...
const users = [
  { id: 'user-ryo', name: 'Ryota Namiki', role: 'visitor' as const },
...
function loadCardsFromFile(): void {
  try {
    if (fs.existsSync(CARDS_FILE)) {
      const data = fs.readFileSync(CARDS_FILE, 'utf8');
      const cardsData: [string, CardRecord][] = JSON.parse(data);
      cardStore.clear();
...
function loadEventsFromFile(): void {
  try {
    if (fs.existsSync(EVENTS_FILE)) {
      ...
      const eventsData: [string, EventMeta][] = JSON.parse(data);
      eventStore.clear();
...
// OCR処理関数（文書構造認識強化版）
async function extractTextFromImage(filePath: string, jobId?: string): Promise<string> {
...
async function extractTextFromPDFWithOCR(filePath: string, jobId?: string): Promise<string> {
...
app.put('/api/cards/:id', (req, res) => {
  try {
    ...
    const {
      ...
      companyName,
      eventId,
      authorId = record.detail.header.author.id,
      ocrText = record.detail.body.ocr.text,
      tags = [],
      memo,
    } = req.body ?? {};
...
// Webクリップ用のメタデータ生成関数
function generateWebClipMetadata(url: string, title: string, description?: string): WebClipMetadata {
...
<head>
    ...
    <!-- Apple Touch Icons -->
    ${webClipMetadata.appleWebApp.icons.map((icon: { sizes: string; src: string }) => 
...
// OCRワーカー（画像・スキャン）
async function processOCRTask(source: any, options: any, jobId?: string) {
  try {
...
async function evaluateOrientationVariants(filePath: string, jobId?: string): Promise<{ bestPath: string; bestAngles: number[]; cleanup: string[] }> {
  const angles = [0, 90, 180, 270];
  const cleanupPaths: string[] = [];
  let bestPath = filePath;
  let bestAngles: number[] = [0];
...
async function tryExtractTextLayer(reader: pdfjsLib.PDFDocumentProxy, pageLimit = 20): Promise<string> {
  let combined = '';
  ...
  for (let pageNum = 1; pageNum <= Math.min(reader.numPages, pageLimit); pageNum++) {
    try {
      ...
      const textContent = await page.getTextContent();
      const pageText = textContent.items
        .map((item: any) => (item.str && item.str.trim() ? item.str.trim() : ''))
        .filter(Boolean)
        .join(' ');
...
type CanvasContextPair = {
  canvas: Canvas;
  context: CanvasRenderingContext2D;
};

class SimpleCanvasFactory {
  create(width: number, height: number): CanvasContextPair {
    const canvas = createCanvas(width, height) as Canvas;
    const context = canvas.getContext('2d') as unknown as CanvasRenderingContext2D;
    return { canvas, context };
  }

  reset(canvasAndContext: CanvasContextPair, width: number, height: number): void {
    canvasAndContext.canvas.width = width;
    canvasAndContext.canvas.height = height;
  }

  destroy(canvasAndContext: CanvasContextPair): void {
    canvasAndContext.canvas = null as unknown as Canvas;
    canvasAndContext.context = null as unknown as CanvasRenderingContext2D;
  }
}

async function renderPdfPageToBuffer(reader: pdfjsLib.PDFDocumentProxy, pageNum: number, dpi = 400): Promise<Buffer | null> {
  try {
    const page = await reader.getPage(pageNum);
    const scale = dpi / 72;
...
async function ocrPdfPages(reader: pdfjsLib.PDFDocumentProxy, jobId: string | undefined, pageLimit = 5): Promise<string> {
  let combined = '';
...
async function extractTextFromPDF(filePath: string, enableOCR: boolean = false, jobId?: string): Promise<string> {
  try {
    const pdfBytes = fs.readFileSync(filePath);
...
const tesseractLogger = (jobId?: string, baseProgress = 0, span = 40) => (m: any) => {
  if (jobId && m.status === 'recognizing text' && typeof m.progress === 'number') {
    updateJobProgress(jobId, baseProgress + m.progress * span);
  }
};

async function recognizeText(imagePath: string, lang: string, options: TesseractRecognizeOptions): Promise<TesseractRecognizeResult> {
  const baseOptions = options as Partial<Tesseract.WorkerOptions>;
  return (Tesseract as any).recognize(imagePath, lang, baseOptions) as Promise<TesseractRecognizeResult>;
}

function getGlobalDocuments(): Map<string, Document> {
  if (!global.documents) {
    global.documents = new Map();
  }
...
function getGlobalJobs(): Map<string, Job> {
  if (!global.jobs) {
    global.jobs = new Map();
  }
  return global.jobs;
...
function getGlobalHybridResults(): Map<string, HybridOCRResult> {
  if (!global.hybridResults) {
    global.hybridResults = new Map();
  }
...
function findActiveSegments(values: number[], minActive: number, minLength: number): Array<{ start: number; end: number; score: number }> {
...
function determineBlockPSM(width: number, height: number): { type: 'paragraph' | 'line' | 'block'; psm: string } {
  if (height <= 60) {
...
async function detectLayoutBlocks(binaryImagePath: string): Promise<LayoutDetectionResult> {
  try {
    ...
    const horizontalProjection = new Array<number>(height).fill(0);
    ...
    for (const segment of rowSegments) {
      ...
      const verticalProjection = new Array<number>(width).fill(0);
